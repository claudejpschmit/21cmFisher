...
COMP = mpicc
CXX = $(COMP) -Wall -std=c++11 -Wno-deprecated 
...
LINKER = -L$(BOOST_HOME)/libs #-lboost_filesystem

# Linking Armadillo
INCLUDES += -I$(ARMADILLO_HOME)/build/armadillo-6.200.2/include/ -DARMA_DONT_USE_WRAPPER
#INCLUDES += -I$(ARMADILLO_HOME)/include -DARMA_DONT_USE_WRAPPER
LINKER += -L$(ARMADILLO_HOME)/lib

# Linking GSL
INCLUDES += -I$(GSL_HOME)/include/
LINKER += -L$(GSL_HOME)/lib/

LINKER += -L$(INTEL_HOME)/mkl/lib/intel64/libmkl_rt.so
#LINKER += -L/usr/lib64/libblas.so.3

# Linking Wignersymbols
INCLUDES += -I/home/cjs213/WignerSymbols_library/include/
LINKER += -L/home/cjs213/WignerSymbols_library/lib/ 
