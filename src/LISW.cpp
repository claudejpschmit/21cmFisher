        ...
        double z1, double z2, double z3)
{
    ...
    term1 = W1 * Cl(l2, z1, z2) * Ql(l3, min_z1z3); 
    term2 = W2 * Cl(l3, z2, z3) * Ql(l1, min_z1z2); 
    term3 = W3 * Cl(l1, z3, z1) * Ql(l2, min_z2z3); 
    term4 = W4 * Cl(l3, z1, z3) * Ql(l2, min_z1z2); 
    term5 = W5 * Cl(l1, z2, z1) * Ql(l3, min_z2z3); 
    term6 = W6 * Cl(l2, z3, z2) * Ql(l1, min_z1z3); 
...
double Bispectrum_LISW::W_lll_mmm(int l1, int l2, int l3, int m1, int m2, int m3)
{
    ...
    double Gaunt_integral = sqrt((2*l1+1)*(2*l2+1)*(2*l3+1)/(4*pi)) * W3J1 * W3J2;
    return 0.5 * pow(-1, m1+m2+m3) * (l3*(l3+1) + l2*(l2+1) - l1*(l1+1)) * Gaunt_integral;
}
...
double Bispectrum_LISW::P_phi(double k, double z)
{
    ...
    return pre * pow(1+z,2) * analysis->model->Pkz_interp(k,z,0);
}
