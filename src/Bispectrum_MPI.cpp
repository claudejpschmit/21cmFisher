...
Bispectrum::Bispectrum(AnalysisInterface* analysis, MPI_Comm communicator)
{
...
vector<double> Bispectrum::give_interp_theta_vals(int li, int lj, int q, int Pk_i, int Tb_i, int q_i,\
        double zc_max, double zc_min, double delta_zc)
{
    /////////////////////////
    double delta_z_loc = 0.1;
...
vector<Theta> Bispectrum::make_interps(int l, int size, double* vals, double zc_max, double zc_min, double delta_zc)
{
    /////////////////////////
    double delta_z_loc = 0.1;
    double zmin = zc_min - 1.5 * delta_z_loc;
    ...
    /////////////
    real_1d_array v_zs, v_zcs, v_vals;
    v_zs.setlength(z_steps+1);
    v_zcs.setlength(zc_steps+1);
    int vals_size = (zc_steps+1)*(z_steps+1);
    v_vals.setlength(vals_size);
    ...
    for (int Pk_i = 0; Pk_i < analysis->model->Pkz_size(); Pk_i++)
    {
        for (int Tb_i = 0; Tb_i < analysis->model->Tb_size(); Tb_i++)
        {
            ...
            {
                index++;
                for (unsigned int i = 0; i < vals_size; i++){
                    v_vals[i] = vals[index * vals_size + i];
                }

                spline2dinterpolant interp;
                spline2dbuildbicubicv(v_zs, z_steps+1, v_zcs, zc_steps+1, v_vals, 1, interp);
